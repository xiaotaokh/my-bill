function getSpaceClass(direction, wrap, align, justify) {
  var classes = ['ant-space'];

  // 方向类名
  classes.push("ant-space-".concat(direction));

  // 是否换行
  if (wrap && direction === 'horizontal') {
    classes.push('ant-space-wrap');
  }

  // 对齐方式
  if (align) {
    classes.push("ant-space-align-".concat(align));
  }

  // 主轴对齐方式
  if (justify) {
    classes.push("ant-space-justify-".concat(justify));
  }
  return classes.join(' ');
}
function getSpaceStyle(size) {
  var gap;

  // 处理间距大小
  switch (size) {
    case 'small':
      gap = '0.16rem';
      break;
    case 'large':
      gap = '0.48rem';
      break;
    case 'middle':
    default:
      gap = '0.32rem';
  }

  // 如果size是数字，直接使用
  if (!isNaN(size)) {
    gap = size / 100 + 'rem';
  }
  return "gap: ".concat(gap);
}
export default {
  getSpaceClass: getSpaceClass,
  getSpaceStyle: getSpaceStyle
};